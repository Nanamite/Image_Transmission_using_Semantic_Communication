...
def calculate_ber(bitstream_original, bitstream_received):
    total_bits = len(bitstream_original)
    error_bits = sum(original_bit != received_bit for original_bit, received_bit in zip(bitstream_original, bitstream_received))
    ber = error_bits / total_bits
    return ber

def tx_rx(section, coords, snr= 5, K= 1, background_num= 1, person_num= 1, image_num= 1):
    x = coords[0]
    y = coords[1]
    w = coords[2]
    h = coords[3]
    ...
    with open('ber_seg.csv', 'a') as f:
        f.write(f'{background_num}, {person_num}, {image_num}, {K}, {calculate_ber(bitstream, bitstream_received)}, {snr}\n')
